...
typedef enum _EPAT_CMI_DEV_PRIM_ID_TAG
{
    ...
    CMI_DEV_IMS_START_PS_REQ,               //CmiDevImsStartPsReq
    CMI_DEV_IMS_START_PS_CNF,

    CMI_DEV_IMS_DEREG_IND,                  //CmiDevImsDeregInd
    CMI_DEV_IMS_DEREG_RSP,                  //CmiDevImsDeregRsp
...
typedef struct CmiDevSetExtCfgReq_Tag
{
    ...
    */
    ...
    BOOL    enableSpsPresent;
    BOOL    enableSps;          //False: disable Semi-PersisterSched capability

    BOOL    enableTtiBundlingPresent;
    BOOL    enableTtiBundling;  //False: disable TTI-Bundling capability
    UINT8   rsvd5;
    BOOL    rohcProfileBitPresent;
    UINT16  rohcProfileBit;     //BIT8->BIT0: profile0x0104,profile0x0103,profile0x0102,profile0x0101,profile0x0006,profile0x0004,profile0x0003,profile0x0002,profile0x0001
...
typedef struct CmiDevPhyStatusInfo_Tag
{
    ...
    INT8        txPower;
    UINT8       pMax;//max tx pwr, phy get from sib1
...
******************************************************************************/
typedef enum CmiSetPsTestEnum_TAG
{
    CMI_DEV_SET_PS_TEST_MAC = 0,
    CMI_DEV_SET_PS_TEST_RLC,
    CMI_DEV_SET_PS_TEST_PDCP,
    CMI_DEV_SET_PS_TEST_ERRC,
    CMI_DEV_SET_PS_TEST_EMM,
    CMI_DEV_SET_PS_TEST_ESM,
    CMI_DEV_SET_PS_TEST_TCPIP,
    CMI_DEV_SET_PS_TEST_ALL,
    CMI_DEV_DISABLE_PS_TEST
}CmiSetPsTestEnum;
...
{
    UINT8   setPsTestType;   //CmiSetPsTestEnum
    UINT8   reserved0;
    UINT16  reserved1;
}
CmiDevSetPsTestReq;
...
{
    UINT16  psTestTaskBitmap;   /* bit 0 - MAC, bit 1 - RLC, bit 2 - PDCP, bit 3 - ERRC, bit 4 - EMM, bit 5 - ESM, bit 6 - TCPIP */
    UINT16  reserved;
}
CmiDevGetPsTestCnf;
...
/******************************************************************************
 * CMI_DEV_IMS_START_PS_REQ/CMI_DEV_IMS_START_PS_CNF
 * IMS stack is ready, and PS could be started (PLMN search and register)
 * Note: if IMS enabled, in followeing case, PS will wait for this msg to start PS
 *  a) first hardware power on
 *  b) software re-start: cfun0 then cfun1
 *  c) SIM refresh, if ISIM/IMSI changes
******************************************************************************/
typedef CamCmiEmptySig  CmiDevImsStartPsReq;

typedef CamCmiEmptySig  CmiDevImsStartPsCnf;


/******************************************************************************
 * CMI_DEV_IMS_DEREG_IND/CMI_DEV_IMS_DEREG_RSP
 * PS need to detach (or IMS PDN need to deact), need to notify IMS to deregister
 *  firstly.
 * Note:
 *  Since PS don't know whether IMS is real registered or not, the PS always notifies
 *   the IMS to de-register, IMS should reply with "CmiDevImsDeregRsp" no matter
 *   whether IMS de-registered is triggered or not. And PS is waiting for
 *   "CmiDevImsDeregRsp" to do next action.
******************************************************************************/
typedef enum CmiDevImsDeregCause_enum
{
    CMI_IMS_DEREG_CAUSE_INVALID     = 0,
    CMI_IMS_DEREG_SWITCH_OFF        = 1,    /* CFUN0/CFUN4 */
    CMI_IMS_DEREG_MO_PS_DETACH      = 2,    /* MO PS detach, triggered by AT+CGATT=0, or AT+COPS=2 */
    CMI_IMS_DEREG_MO_IMS_PDP_DEACT  = 3,    /* MO IMS default bearer deacted, triggered by AT+CGACT */
    CMI_IMS_DEREG_SIM_REMOVE        = 4,    /* SIM removed */
    CMI_IMS_DEREG_SIM_REFRESH       = 5     /* SIM refresh (sim file changes), which need to re-attach */
}CmiDevImsDeregCause;

typedef struct  CmiDevImsDeregInd_Tag
{
    UINT8       deregCause;     /* CmiDevImsDeregCause */
    UINT8       rsvd0;
    UINT16      rsvd1;
}CmiDevImsDeregInd;

typedef CamCmiEmptySig  CmiDevImsDeregRsp;
