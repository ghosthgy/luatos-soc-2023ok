 ...
 * Description:  EC718 platform power management header file
...
typedef enum PmuVoteDozeModule_enum
{
    PMU_DOZE_UICC_MOD,
...
//list which module should vote before system enter slp1 state
...
/*
 * list which module should vote before system enter slp2/hib state
 */
...
void apmuVoteToDozeState(PmuVoteDozeModule pmuMod, BOOL bAllow);
...
void apmuGetDozeVoteDetail(uint32_t *dozeVoteFlag);
...
void apmuUpdateCPPllState(bool state);
uint32_t apmuGetHibSecondCount(void);
void apmuCpmuIntMask(void);
void apmuCpmuIntRestore(void);
void apmuStartAdcProxy(void);
